...
Optionally enable **conformal prediction** with a target error rate **α** (default 0.1 ⇒ 90% coverage) to attach calibrated, distribution-free uncertainty to each prediction. For regression, each value is shown with a `(1−α)` **prediction interval** and the CSV gains `pi_low_<task>` / `pi_high_<task>` columns; over many molecules the true value is expected to fall inside the interval about `(1−α)` of the time. For classification, each prediction is labelled **Positive** (confidently active), **Negative** (confidently inactive), or **Uncertain** (neither can be ruled out), and the CSV gains `conformal_<task>`, `in_set_<task>`, and `out_set_<task>` columns. Conformal takes precedence over the ensemble ± std when both are available. It requires a calibration set saved at training time, so checkpoints trained with conformal disabled (or uploaded without one) fall back to standard predictions with a notice.
